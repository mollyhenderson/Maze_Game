...
public class GridCreator : MonoBehaviour {
			...
			if (isEmpty) { 
				...
				foreach (Transform cell in Grid) {
					...
					if (!PathCells.Contains(cell)) {
						...
						cell.localScale = new Vector3(1f,5f,1f); 
	...
	private void setPerimeter(){
		for (int x=-1; x<=this.Size.x; x++) {
			...
			newCell.parent = transform;
			newCell.GetComponent<CellScript>().Position = new Vector3(x, 0, z);
			newCell.localScale = new Vector3(1f, 4f, 1f);
			newCell.localPosition = new Vector3(newCell.transform.localPosition.x, 1.5f, newCell.transform.localPosition.z);
			newCell.GetComponentInChildren<TextMesh>().renderer.enabled = false;
			newCell.renderer.material.color = Color.red;
			z = (int)this.Size.z;
			newCell = (Transform)Instantiate(CellPrefab, new Vector3(x, 0, z), Quaternion.identity);
			newCell.name = string.Format("({0},0,{1})", x, z);
			newCell.parent = transform;
